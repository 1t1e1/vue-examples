            ...
            handler: function() {
                ...
                this.garbleInterval = setInterval(() => {
                    ...
                    for (var i = 0; i < this.text.length; i++) {
                        if (
                            this.text.charAt(i) != " " &&
                            this.text.charAt(i) != this.textRendered.charAt(i)
                        ) {
                            if (Math.random() > this.chance) {
                                //another random
                                this.textRendered = this.replaceAt(
                                    this.textRendered,
                                    i,
                                    Math.random()
                                        .toString(36)
                    ...
                    if (this.textRendered === this.text)
                        clearInterval(this.garbleInterval);
                }, this.time);
            },
        },
